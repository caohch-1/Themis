...
//        localPairs();
    ...
    private static void findAllPropertiesMultiUsage(String systemName, String systemPath) throws XPathExpressionException, IOException, ParserConfigurationException, SAXException {
        ...
        Map<String, Map<String, FileAddressExtraction.PropertyHelper>> xmlProperties = parseXMLProperties(xmlFiles);
        Set<FileAddressExtraction.PropertyHelper> xmlConfigurationProperties = new HashSet<>();
        ...
        Map<FileAddressExtraction.PropertyHelper, Set<Pair<SootMethod, Stmt>>> propertyUsage = new HashMap<>();
        for (StringConstant property : configPropertyUsage.keySet()) {
            String propertyName = property.value;
            for (FileAddressExtraction.PropertyHelper propertyHelper : xmlConfigurationProperties) {
    ...
    public static Map<FileAddressExtraction.PropertyHelper, Set<Pair<SootMethod, Stmt>>> findFilePathInfo(Set<FileAddressExtraction.PropertyHelper> filePaths, Map<StringConstant, Set<Pair<SootMethod, Stmt>>> configPropertyUsage, String systemName) throws IOException {
        Map<FileAddressExtraction.PropertyHelper, Set<Pair<SootMethod, Stmt>>> addressPropertyUsage = new HashMap<>();
        for (StringConstant property : configPropertyUsage.keySet()) {
            String propertyName = property.value;
            for (FileAddressExtraction.PropertyHelper propertyHelper : filePaths) {
                if (propertyName.equals(propertyHelper.name)) {
                    addressPropertyUsage.put(propertyHelper, configPropertyUsage.get(property));
                }
            }
...
//            }
    ...
    public static void saveFilePathPropertyUsage(Map<FileAddressExtraction.PropertyHelper, Set<Pair<SootMethod, Stmt>>> addressPropertyUsage, String filePath) throws IOException {
        if (Files.exists(Paths.get(filePath))) {
            return;
        }

        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath))) {
            for (Map.Entry<FileAddressExtraction.PropertyHelper, Set<Pair<SootMethod, Stmt>>> entry : addressPropertyUsage.entrySet()) {
                FileAddressExtraction.PropertyHelper propertyHelper = entry.getKey();
                Set<Pair<SootMethod, Stmt>> methodStmtPairs = entry.getValue();
