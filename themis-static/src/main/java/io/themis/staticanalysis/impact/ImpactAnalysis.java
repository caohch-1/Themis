...
import indi.dc.access.SameClassVariableAccessAnalysis;
import indi.dc.rw.RWSameAccessSite;
import indi.dc.rw.ReadWriteAnalysis;
...
public class ImpactAnalysis {
    public static Set<ImpactRWSameAccessSite> getImpactSitesWhole(Set<RWSameAccessSite> rwSameAccessSites) {
        ...
        for (RWSameAccessSite rwSameAccessSite : rwSameAccessSites) {
            ...
            for (ImpactRWSameAccessSite impactRW : temp) {
                Log.i("============================================================================================");
    ...
    // 如果rootCauseVariable是Static 1)考虑其他所有函数来get这个var从而lead到symptom (?不合理暂缓)
    //                              2)有已经检测到的call chain，向上只沿着这条call chain检测，即processReturnValueImpact需要限定在这条call chain上
    public static Set<ImpactRWSameAccessSite> getImpactSitesSameAccess(RWSameAccessSite rwSameAccessSite) {
        ...
        if (rwSameAccessSite.sameAccessClsVar != null) {
            ...
            if (RPCUtils.isRPCImplField(rootCauseField)) {
                ...
                for (SootMethod potientialSymptomMethod : rootCauseField.getDeclaringClass().getMethods()) {
                    ...
                    Map<SootField, Set<Stmt>> fieldAccessMap = SameClassVariableAccessAnalysis.getClsVarAccessMap(potientialSymptomMethod);
        ...
        } else if (rwSameAccessSite.sameAccessLocalVar != null) {
            symptomSites.addAll(getImpactSitesAccessLocal(rwSameAccessSite.accessSite1));
        }
    ...
    public static Set<ImpactRWSameAccessSite.SymptomSite>  getImpactSitesAccessLocal(AccessSite accessSite) {
        Local rootCauseLocalVar = accessSite.accessVariableLocal;
        ...
        try {
            ...
            fieldLocals.add(rootCauseLocalVar);

            for (Unit unit : jimpleBody.getUnits()) {
                if (unit instanceof JAssignStmt) {
                    JAssignStmt assign = (JAssignStmt) unit;
                    if (assign.getRightOp() instanceof Local || assign.getRightOp() instanceof ArrayRef) {
                        fieldLocals.add(assign.getLeftOp());
                    }
                }
            }

            analyzeMethodImpactLocal(rootCauseMethod, fieldLocals, symptomSites, new HashSet<>(), 0, new ArrayList<>(), new ArrayList<>());
...
//            Log.i("[Error] ", "In ", new Object(){}.getClass().getEnclosingMethod().getName(), ": ", e.getMessage().replace("\n", ""));
    ...
    public static Set<ImpactRWSameAccessSite.SymptomSite>  getImpactSitesAccess(AccessSite accessSite) {
        ...
        try {
            ...
            if (rootCauseClsVar.isStatic()) {
                for (Unit unit : jimpleBody.getUnits()) {
                    if (unit instanceof JAssignStmt) {
                        JAssignStmt assign = (JAssignStmt) unit;
                        if (assign.getRightOp() instanceof StaticFieldRef) {
                            StaticFieldRef fieldRef = (StaticFieldRef) assign.getRightOp();
                            if (ReadWriteAnalysis.isValidStaticAccess(fieldRef, rootCauseClsVar, declaringClass)) {
                                fieldLocals.add((Local) assign.getLeftOp());
                            }
                        }
                    }
                }
                List<Analyzer.CallSite> rpcCallSites = accessSite.callSites2RPC;
                if (rpcCallSites != null && !rpcCallSites.isEmpty()) {
                    analyzeMethodImpactStatic(rootCauseMethod, fieldLocals, rootCauseClsVar, symptomSites, new HashSet<>(), 0, new ArrayList<>(), new ArrayList<>(), rpcCallSites);
                }
                ...
                analyzeMethodImpact(rootCauseMethod, fieldLocals, rootCauseClsVar, symptomSites, new HashSet<>(), 0, new ArrayList<>(), new ArrayList<>());
            }
        } catch (Exception e) {
//            Log.i("[Error] ", "In ", new Object(){}.getClass().getEnclosingMethod().getName(), ": ", e.getMessage().replace("\n", ""));
            e.printStackTrace();
    ...
    public static void analyzeMethodImpactStatic(SootMethod method, Set<Value> paramLocals, SootField targetField, Set<ImpactRWSameAccessSite.SymptomSite> results,Set<SootMethod> visitedMethods, int depth, List<SootMethod> callChian, List<Stmt> rpcCallSiteChain, List<Analyzer.CallSite> callSites2RPC) {
        // 终止条件检查
...
//            Log.i("[Warn] ", "Max recursion depth reached for: " + method.getName());
        ...
        try {
            ...
            Stmt startStmt = null;
            for (Analyzer.CallSite callSite : callSites2RPC) {
                if (callSite.getCallMethod().getSignature().equals(method.getSignature())) {
                    startStmt = callSite.getCallStmt();
                }
            }

            // 分析当前method中的ifStmt是否涉及兴趣变量以及导致symptom
            analyzeConditionsInMethodStatic(startStmt, body, paramLocals, targetField, results, cfg, callChian, rpcCallSiteChain);

            // 分析当前method中的invokeStmt是否涉及兴趣变量，并进入其中递归分析
            if (startStmt != null) {
                List<Unit> succStmts = cfg.getSuccsOf(startStmt);
                Set<Unit> visitedStmts =  new HashSet<>();
                while (!succStmts.isEmpty()) {
                    Unit current = succStmts.remove(0);
                    if (visitedStmts.contains(current)) continue;
                    visitedStmts.add(current);
                    if (((Stmt) current).containsInvokeExpr()) {

                        processMethodInvokeStatic((Stmt) current, paramLocals, targetField, results, new HashSet<>(visitedMethods), depth, callChian, rpcCallSiteChain, callSites2RPC);
                    }
                    succStmts.addAll(cfg.getSuccsOf(current));
                }
            } else {
                for (Unit unit : body.getUnits()) {
                    if (((Stmt) unit).containsInvokeExpr()) {
                        processMethodInvokeStatic((Stmt) unit, paramLocals, targetField, results, new HashSet<>(visitedMethods), depth, callChian, rpcCallSiteChain, callSites2RPC);
                    }
                }
            }

            // 分析当前method中的返回值是否涉及兴趣变量，如果涉及，寻找当前method的caller并进一步分析返回值是否会引起symptom
            for (Unit unit : body.getUnits()) {
                if (unit instanceof ReturnStmt) {
                    ...
                    if (isValueRelated(returnValue, paramLocals, targetField)) {
                        processReturnValueImpactStatic(method, targetField, results, new HashSet<>(visitedMethods), depth, callChian, rpcCallSiteChain, callSites2RPC);
                    }
                }
            }
...
//            Log.i("[Error] ", "In ", new Object(){}.getClass().getEnclosingMethod().getName(), ": ", e.getMessage().replace("\n", ""));
    ...
    private static void processReturnValueImpactStatic(SootMethod calleeMethod, SootField targetField, Set<ImpactRWSameAccessSite.SymptomSite> results, Set<SootMethod> visitedMethods, int depth, List<SootMethod> callChain, List<Stmt> rpcCallSiteChain, List<Analyzer.CallSite> callSites2RPC) {
        Analyzer.CallSite callerSite = null;
        for (int i = 0 ; i < callSites2RPC.size() ; i++) {
            if (callSites2RPC.get(i).getCallMethod().getSignature().equals(calleeMethod.getSignature())) {
                if (i >= 1) callerSite = callSites2RPC.get(i - 1);
            }
        }
        if (callerSite != null) {
            Stmt callSiteStmt = callerSite.getCallStmt();
            SootMethod callerMethod = callerSite.getCallMethod();
            ...
            if (callSiteStmt instanceof AssignStmt) {
                ...
                if (leftOp instanceof Local || leftOp instanceof ArrayRef) {
                    ...
                    analyzeMethodImpactStatic(callerMethod, newParamLocals, targetField, results, new HashSet<>(visitedMethods), depth + 1, newCallChain, rpcCallSiteChain, callSites2RPC);
                } else if (leftOp instanceof InstanceFieldRef) {
                    InstanceFieldRef instanceFieldRef = (InstanceFieldRef) leftOp;
                    SootField callerField = instanceFieldRef.getField();

                    // 构建新的调用链并递归分析调用者方法
                    List<SootMethod> newCallChain = new ArrayList<>(callChain);
                    newCallChain.add(callerMethod);
                    analyzeMethodImpactStatic(callerMethod, new HashSet<>(), callerField, results, new HashSet<>(visitedMethods), depth + 1, newCallChain, rpcCallSiteChain, callSites2RPC);
                } else if (leftOp instanceof StaticFieldRef) {
                    StaticFieldRef callerFieldRef = (StaticFieldRef) leftOp;
    ...
    public static void analyzeMethodImpactLocal(SootMethod method, Set<Value> paramLocals, Set<ImpactRWSameAccessSite.SymptomSite> results,Set<SootMethod> visitedMethods, int depth, List<SootMethod> callChian, List<Stmt> rpcCallSiteChain) {
        // 终止条件检查
        if (depth > MAX_RECURSION_DEPTH) {
            Log.i("[Warn] ", "Max recursion depth reached for: " + method.getName());
            return;
        ...
        try {
            ...
            // 分析当前method中的ifStmt是否涉及兴趣变量以及导致symptom
            analyzeConditionsInMethod(body, paramLocals, null, results, cfg, callChian, rpcCallSiteChain);

            // 分析当前method中的invokeStmt是否涉及兴趣变量，并进入其中递归分析
            for (Unit unit : body.getUnits()) {
                if (((Stmt) unit).containsInvokeExpr()) {
                    processMethodInvokeLocal((Stmt) unit, paramLocals, results, new HashSet<>(visitedMethods), depth, callChian, rpcCallSiteChain);
                }
            }
            ...
            for (Unit unit : body.getUnits()) {
                if (unit instanceof ReturnStmt) {
                    ...
                    Value returnValue = returnStmt.getOp();
                    // 检查返回值是否与兴趣变量相关
                    if (isValueRelated(returnValue, paramLocals, null)) {
                        processReturnValueImpact(method, null, results, new HashSet<>(visitedMethods), depth, callChian, rpcCallSiteChain);
                    }
                }
            }
...
//            Log.i("[Warn] ", "Max recursion depth reached for: " + method.getName());
        ...
        try {
            ...
            for (Unit unit : body.getUnits()) {
                if (((Stmt) unit).containsInvokeExpr()) {
                    processMethodInvoke((Stmt) unit, paramLocals, targetField, results, new HashSet<>(visitedMethods), depth, callChian, rpcCallSiteChain);
                }
            }
...
//            Log.i("[Error] ", "In ", new Object(){}.getClass().getEnclosingMethod().getName(), ": ", e.getMessage().replace("\n", ""));
    ...
    private static void processReturnValueImpact(SootMethod calleeMethod, SootField targetField, Set<ImpactRWSameAccessSite.SymptomSite> results, Set<SootMethod> visitedMethods, int depth, List<SootMethod> callChain, List<Stmt> rpcCallSiteChain) {
        if (RPCUtils.isRPCImplMethod(calleeMethod)) {
            Set<Analyzer.CallSite> callers = RPCUtils.findRPCCallerMethods(calleeMethod);
            for (Analyzer.CallSite callerSite : callers) {
                Stmt callSiteStmt = callerSite.getCallStmt();
                SootMethod callerMethod = callerSite.getCallMethod();
                ...
                if (callSiteStmt instanceof AssignStmt) {
                    ...
                    if (leftOp instanceof Local || leftOp instanceof ArrayRef) {
                        ...
                        List<SootMethod> newCallChain = new ArrayList<>(callChain);
                        newCallChain.add(callerMethod);
                        analyzeMethodImpact(callerMethod, newParamLocals, targetField, results, new HashSet<>(visitedMethods), depth + 1, newCallChain, rpcCallSiteChain);
                    } else if (leftOp instanceof InstanceFieldRef) {
                        InstanceFieldRef callerFieldRef = (InstanceFieldRef) leftOp;
                        SootField callerField = callerFieldRef.getField();

                        // 构建新的调用链并递归分析调用者方法
                        List<SootMethod> newCallChain = new ArrayList<>(callChain);
                        newCallChain.add(callerMethod);
                        analyzeMethodImpact(callerMethod, new HashSet<>(), callerField, results, new HashSet<>(visitedMethods), depth + 1, newCallChain, rpcCallSiteChain);
                    }
                }
            }
        } else {
                ...
                if (callSiteStmt instanceof AssignStmt) {
                    ...
                    if (leftOp instanceof Local || leftOp instanceof ArrayRef) {
                        ...
                        if (targetField != null) {
                            ...
                            analyzeMethodImpactLocal(callerMethod, newParamLocals, results, new HashSet<>(visitedMethods), depth + 1, newCallChain, rpcCallSiteChain);
                        }
                    } else if (leftOp instanceof InstanceFieldRef) {
                        InstanceFieldRef callerFieldRef = (InstanceFieldRef) leftOp;
                        SootField callerField = callerFieldRef.getField();
    ...
    public static void analyzeConditionsInMethodStatic(Stmt startStmt, Body body, Set<Value> currentLocals, SootField targetField, Set<ImpactRWSameAccessSite.SymptomSite> results, UnitGraph cfg, List<SootMethod> callChian, List<Stmt> rpcCallSiteChain) {
        // 造成Exception, Error Log, or System abort
        List<Unit> succStmts = cfg.getSuccsOf(startStmt);
        Set<Unit> visitedStmts =  new HashSet<>();
        while (!succStmts.isEmpty()) {
            ...
            if (current instanceof IfStmt) {
                ...
                if (isConditionRelated(ifStmt.getCondition(), currentLocals, targetField)) {
                    checkExceptionBranch(ifStmt.getTarget(), results, cfg, callChian, rpcCallSiteChain);
                    checkExceptionBranch(body.getUnits().getSuccOf(current), results, cfg, callChian, rpcCallSiteChain);
                }
            }
            succStmts.addAll(cfg.getSuccsOf(current));
        }
    ...
    private static boolean isConditionRelated(Value condition, Set<Value> fieldLocals, SootField rootCauseClsVar) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (current instanceof Local || current instanceof InstanceFieldRef || current instanceof ArrayRef) {
            ...
            if (rootCauseClsVar != null) {
                if (current instanceof JInstanceFieldRef) {
                    JInstanceFieldRef fieldRef = (JInstanceFieldRef) current;
                    if (fieldRef.getField().equals(rootCauseClsVar)) {
                        return true;
                    }
                }
                if (current instanceof StaticFieldRef) {
                    StaticFieldRef ref = (StaticFieldRef) current;
                    if (rootCauseClsVar.equals(ref.getField())) {
                        return true;
                    }
                }
            }
...
//        return Scene.v().getActiveHierarchy().isClassSubclassOf(cls, runtimeExceptionClass);
    ...
    private static void processMethodInvokeLocal(Stmt invoke, Set<Value> callerLocals, Set<ImpactRWSameAccessSite.SymptomSite> results, Set<SootMethod> visitedMethods, int depth, List<SootMethod> callChian, List<Stmt> rpcCallSiteChain) {
        SootMethod callee = invoke.getInvokeExpr().getMethod();
        ...
        if (realCallees == null || realCallees.isEmpty()) {
            // 获取与被调用方法参数对应的敏感变量
            Map<Integer, Local> paramMapping = mapRelevantParameters(invoke.getInvokeExpr(), callerLocals, null, callee);
            if (!paramMapping.isEmpty() && callee.hasActiveBody()) {
                // 构建被调用方法的新参数集合
                Set<Value> calleeParams = new HashSet<>(paramMapping.values());
                // 递归分析
                analyzeMethodImpactLocal(callee, calleeParams, results, new HashSet<>(visitedMethods), depth + 1, callChian, rpcCallSiteChain);
            }
        } else {
            for (SootMethod realCallee : realCallees) {
                // 获取与被调用方法参数对应的敏感变量
                Map<Integer, Local> paramMapping = mapRelevantParameters(invoke.getInvokeExpr(), callerLocals, null, realCallee);
                if (!paramMapping.isEmpty() && realCallee.hasActiveBody()) {
                    // 构建被调用方法的新参数集合
                    Set<Value> calleeParams = new HashSet<>(paramMapping.values());
                    // 递归分析
                    rpcCallSiteChain.add(invoke);
                    analyzeMethodImpactLocal(realCallee, calleeParams, results, new HashSet<>(visitedMethods), depth + 1, callChian, rpcCallSiteChain);
                }
            }
        }
    }

    private static void processMethodInvokeStatic(Stmt invoke, Set<Value> callerLocals, SootField targetField, Set<ImpactRWSameAccessSite.SymptomSite> results, Set<SootMethod> visitedMethods, int depth, List<SootMethod> callChian, List<Stmt> rpcCallSiteChain, List<Analyzer.CallSite> callSites2RPC) {
        SootMethod callee = invoke.getInvokeExpr().getMethod();
        // RPC调用替换
        Set<SootMethod> realCallees = RPCUtils.findRPCImplMethods(invoke);
        ...
        if (realCallees == null || realCallees.isEmpty()) {
            ...
            if (!paramMapping.isEmpty() && callee.hasActiveBody()) {
                ...
                Set<Value> calleeParams = new HashSet<>(paramMapping.values());
                // 递归分析
                analyzeMethodImpactStatic(callee, calleeParams, targetField, results, new HashSet<>(visitedMethods), depth + 1, callChian, rpcCallSiteChain, callSites2RPC);
            }
        } else {
            for (SootMethod realCallee : realCallees) {
                // 获取与被调用方法参数对应的敏感变量
                ...
                if (!paramMapping.isEmpty() && realCallee.hasActiveBody()) {
                    ...
                    // 递归分析
                    rpcCallSiteChain.add(invoke);
                    analyzeMethodImpactStatic(realCallee, calleeParams, targetField, results, new HashSet<>(visitedMethods), depth + 1, callChian, rpcCallSiteChain, callSites2RPC);
                }
            }
        }
    ...
    private static boolean isValueRelated(Value value, Set<Value> currentLocals, SootField targetField) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (targetField != null){// 检查字段访问
                if (current instanceof JInstanceFieldRef && ((JInstanceFieldRef) current).getField().equals(targetField)) {
                    return true;
                }

                if (current instanceof StaticFieldRef && ((StaticFieldRef) current).getField().equals(targetField)) {
                    return true;
                }
            }
